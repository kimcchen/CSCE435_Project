...
- Sample Sort<br/>
    ...
    - <strong>comm Weak Scaling Graph</strong><br/>
        <img src="https://github.com/kimcchen/CSCE435_Project/blob/main/sample_sort/weak_scaling_plots/WeakScalingComm.png" width=33% alt="Weak Scaling Graph comm"><br/>
        The communication component shows poor weak scaling behavior, with execution time steadily increasing as both problem size and process count grow. Starting from 2^16 elements with 2 processes, we see relatively low communication overhead, but this increases dramatically at higher scales. There's a particularly sharp increase beyond 256 processes, reaching maximum times at 1024 processes with 2^26 elements. The perturbed and random data types show more erratic patterns with occasional spikes, while sorted and reverse sorted data exhibit slightly more consistent scaling behavior.<br/>
    ...
    - <strong>comp_large Weak Scaling Graph</strong><br/>
        <img src="https://github.com/kimcchen/CSCE435_Project/blob/main/sample_sort/weak_scaling_plots/WeakScalingCommLarge.png" width=33% alt="Weak Scaling Graph comp_large"><br/>
        The computation portion shows better weak scaling characteristics than communication, but still exhibits non-ideal behavior. The execution time shows a sawtooth pattern with local peaks and valleys, suggesting varying computational efficiency at different scales. Sorted data consistently performs best (lowest times), while reverse sorted and perturbed data typically require more computation time. There's a dramatic increase in computation time at 1024 processes (2^26 elements) for all input types, indicating a potential computational bottleneck at large scales.<br/>
    ...
    - <strong>main Weak Scaling Graph</strong><br/>
        <img src="https://github.com/kimcchen/CSCE435_Project/blob/main/sample_sort/weak_scaling_plots/WeakScalingMain.png" width=33% alt="Weak Scaling Graph main"><br/>
        The main execution time combines both communication and computation effects, showing a generally increasing trend with scale. The pattern is more stable than either component alone, suggesting some overlap between communication and computation. All input types follow similar trends until around 32 processes, after which they begin to diverge. The final scaling to 1024 processes shows the most significant performance degradation, with execution times increasing by roughly an order of magnitude from the smallest scale. This indicates that the algorithm's overall efficiency decreases substantially at larger scales, regardless of input type.<br/>
    ...
    - <Strong>Final Sample Sort Remarks</strong><br/>
        The analysis of communication (comm), computation (comp_large), and overall performance (main) reveals significant scaling challenges in this sample sort parallel implementation. The communication component shows severe scaling issues with dramatically increasing overhead beyond 256 processes in weak scaling, while computation exhibits a more consistent but still non-ideal sawtooth pattern before sharply degrading at 1024 processes. These scaling behaviors stem from increasing cache misses indicating poor data locality, inefficient data partitioning evidenced by divergent behavior among input types (particularly in perturbed and random data), and excessive inter-processor communication becoming problematic at higher scales. The overall performance shows a steady degradation with execution times increasing by an order of magnitude at the largest scales, suggesting fundamental efficiency issues regardless of input type. Improvements should focus on optimizing communication patterns at high process counts, implementing better data partitioning strategies, and enhancing cache utilization through improved data locality.<br/>
...
- Radix Sort<br/>
    ...
    - <strong>Comm Speedup Graphs</strong><br/>
        ...
        <br/>
    - <strong>Comm Weak Scaling Graph</strong><br/>
        <img width="33%" alt="weakScalingComm" src="https://github.com/user-attachments/assets/4910dd4b-1e27-41a8-92cf-ae027b7599df"><br/>
